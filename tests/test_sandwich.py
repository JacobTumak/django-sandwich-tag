...
class TestSandwichTag(SimpleTestCase):
    ...
    def _sandwich_with_fixings(
        ...
        bread_template = (
            bread if bread_as_var and bread else f"'{bread}'" if bread else ""
        )
        title = (
            f" title='{self.default_title if title is None else title}'"
            if with_title
            else ""
        )
        return Template(
    ...
    def test_with_template_object(self):
        ...
        rendered = self._sandwich_with_fixings(
            "bread_template", bread_as_var=True, title="", fixings=""
        ).render(
    ...
    def test_with_template_name(self):
        with_sandwich = self._sandwich_with_fixings(self.sw_template_name).render(
            Context({})
        )
        manual = get_template(
            self.sw_template_name
        ).template.render(  # not treated as a sandwich
            Context(
                dict(
                    title=self.default_title,
                    sandwich_fixings=SafeString(self.default_fixings),
                )
            )
        )
    ...
    def test_sandwich_context_isolation(
        self,
    ):  # alternate name: test_sandwich_context_behaviour
        """
        Ensure that the...
    ...
    def test_resolved_kwargs_not_cached(self):
        ...
        expected = (
            "<h1>title1</h1><div></div>"
            "<h1>title2</h1><div></div>"
            "<h1>title3</h1><div></div>"
        )
        rendered = Template(
            "{% load sandwich %}"
    ...
    def test_sandwich_renders_children(self):
        ...
        self.assertHTMLEqual(
            rendered, "<h1>default title text</h1><div>default body text</div>"
        )
